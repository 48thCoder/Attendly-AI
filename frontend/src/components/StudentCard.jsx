...
import { User, MoreVertical, Trash2, Eye } from 'lucide-react';
import { useState } from 'react';
import { getInitials, getAvatarColor, getAttendanceColor, getAttendanceBg } from '../utils/helpers';
    ...
    >
      <div className="flex items-start justify-between mb-4">
        <div className="flex items-center gap-3">
          <div className={`w-12 h-12 rounded-xl bg-gradient-to-br ${gradientClass} flex items-center justify-center flex-shrink-0 font-playfair font-bold text-white text-sm shadow-lg`}>
            {initials}
          </div>
          <div>
            <h4 className="font-semibold text-white group-hover:text-primary transition-colors leading-tight">{student.name}</h4>
            <p className="text-xs text-gray-500 font-mono mt-0.5">{student.roll}</p>
          </div>
        </div>
          ...
          {menuOpen && (
            <>
              ...
              <div className="absolute right-0 top-8 z-20 bg-surfaceLight border border-surfaceLight rounded-xl shadow-xl w-40 py-1 text-sm">
                <button className="w-full flex items-center gap-2 px-3 py-2 hover:bg-background hover:text-primary transition-colors text-gray-300">
                  <Eye size={14} /> View Profile
                </button>
                {onDelete && (
                  <button
                    onClick={() => { onDelete(student.id); setMenuOpen(false); }}
      ...
      <div className="space-y-3">
        ...
        <div>
          <div className="flex items-center justify-between mb-1.5">
            ...
            <span className={`text-sm font-semibold font-playfair ${attColor}`}>{pct}%</span>
          </div>
